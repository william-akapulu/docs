...
3. The provider joins the **Daily** room for realtime media. Install `@daily-co/daily-react` (and peer `@daily-co/daily-js`) and use its primitives to draw video—for example `DailyVideo`, `useDaily`, and `useVideoTrack`. For assistant video selection, use `useAkapuluParticipantRoles` from `@akapulu/react`.

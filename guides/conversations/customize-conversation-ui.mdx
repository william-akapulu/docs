...
function CustomConversationUI() {
  ...
  async function startConversation() {
    ...
    // Akaulu returns room_url, token, and conversation_session_id
...
<Note>
On macOS, iPhone Continuity Camera can sometimes be selected automatically as the active camera or microphone. If your phone is in your pocket or far away, this can lead to muffled audio quality. We recommend explicitly selecting your intended input devices in your app before starting the call, and turning off Continuity Camera on iPhone when you are not using it (**Settings → General → AirPlay & Continuity → Continuity Camera**).
</Note>
...
- `message.body`: HTTP tool payload/body(headers and body)
...
Here's a single snippet showing all the above in a single tempalte file
...
async function connectConversation(apiKey: string): Promise<ConnectResponse> {
  const response = await fetch("https://akapulu.com/api/conversations/connect/", {
    ...
    body: JSON.stringify({
      // TODO: add your connect request payload fields.
    }),
  });

...
This guide is based on the [Akapulu Labs Web SDK](/web-sdk/overview).

## Packages

Install these:

- `@akapulu/server` on your backend
- `@akapulu/react` in the React app
- `@akapulu/react-ui` only if you use the prebuilt `AkapuluConversation`
- `@daily-co/daily-react` only if your own components import `DailyVideo` or Daily hooks
...
The SDK always calls your `updatesPath` with `conversation_session_id` in the URL query string. It polls that route only while the avatar is booting, then stops. Authenticate those GETs with `config.headers` the same way you authenticate connect.
...
- **On your server:** those routes use `@akapulu/server`, read the Akapulu Labs API key from a server-side environment variable, and call the Akapulu Labs HTTP API. That way your API key stays on the server and does not get exposed in the browser.

Authenticate users on your connect **and** updates routes the same way you authenticate the rest of your product. Connect starts a billed conversation, so only authenticated users should be able to start a call. Pass those auth headers with `config.headers` (object or function); do not put them in `connectBody`. The SDK sends those headers on the connect POST and on each boot-time updates GET.
...
## Passing a body and headers to your local connect route

`AkapuluProvider` can send JSON to your local `connectPath` via `config.connectBody`, and headers on connect and updates via `config.headers`.

Each can be a plain object, or a function that is called when the request is sent (not when the provider mounts). Use a function when the value can change or you need to load it asynchronously — for example an auth token. `getAccessToken` below is your own helper.
...
- If `headers` is provided, those headers are sent on the connect POST and on each updates GET.

`config` has no API key field and no fetch interceptor. User auth is `headers`; `connectBody` is payload only. Functions for `connectBody` and `headers` run when the request is sent. `endpoints` and `transport` stay from first mount. The full `AkapuluConfig` type is on the [React SDK](/web-sdk/react-sdk) page.
...
| `durationWarning` | Red banner on the video: session limit reached, upgrade for more minutes | `akapulu-duration-warning` | `duration-warning` |
...
- `playConnectChime` to play a short chime when the call goes live (default `true`; pass `false` to disable)
...
`playConnectChime` behavior:

- default: `true`
- pass `false` to disable
- plays when session status goes `connecting` → `connected` (loading screen ends)
...
Fired once, about 30 seconds before a live call hits its duration cap. The prebuilt `AkapuluConversation` shows a red banner: the call is ending because the plan's session limit was reached, and to upgrade for longer calls and more minutes. The call stays connected. Rooms of 30 seconds or less do not send it.

| Field | Type | Description |
| --- | --- | --- |
| `type` | `"duration_warning"` | Discriminator |
| `secondsRemaining` | `number` | Whole seconds until the call ends |
...
function ConversationEventBridge() {
  useAkapuluEvents((event) => {
    ...
    if (event.type === "duration_warning") {
      // event.secondsRemaining — about 30 seconds left; call is still connected
      return;
    }
    if (event.type === "timeout") {
      // event.reason:
      // - "duration_limit_reached" => call ended because session duration cap was reached
      // - "participant_join_timeout" => call ended because no participant joined in time
      return;
    }
  });
...
3. The provider joins the **Daily** room for realtime media. `@akapulu/react` already depends on `@daily-co/daily-react`. Add it as a **direct** dependency too if you import `DailyVideo`, `useDaily`, or `useVideoTrack` in your own files (so the import is yours, not a nested package). For assistant video selection, use `useAkapuluParticipantRoles` from `@akapulu/react`.

```tsx
import { useAkapuluParticipantRoles } from "@akapulu/react";
...
function BotVideoTile() {
  ...
  const track = useVideoTrack(akapuluBotParticipantId);
  if (!akapuluBotParticipantId || track.isOff) return null;
  return <DailyVideo sessionId={akapuluBotParticipantId} type="video" />;
}
...
6. Call `useConnectChime()` to play the same connect chime as `AkapuluConversation` when status goes `connecting` → `connected`. Skip this hook if you do not want a chime.

```tsx
useConnectChime();
```

7. Optionally `useAkapuluEvents(callback)` to react to the full event stream (`transcript_updated`, `tool_event`, `node_changed`, `timeout`, etc.) while still rendering your own UI.
...
function VideoTile(props: { sessionId: string; isLocal: boolean }) {
  ...
  // `sessionId` must be this participant’s Daily `session_id` (from `useAkapuluParticipantRoles` or `participants().local`).
  return <DailyVideo sessionId={props.sessionId} type="video" />;
}

function CustomCallUi() {
  // Akapulu Labs session: status, lifecycle controls, transcript + progress state.
  const {
    status,
    start,
    end,
  ...
  // Daily participant roles + local self.
  const daily = useDaily();
  // Assistant video participant id selected from Daily roles.
  const { akapuluBotParticipantId } = useAkapuluParticipantRoles();
      ...
      return (
        <>
          ...
          {akapuluBotParticipantId ? (
            <VideoTile sessionId={akapuluBotParticipantId} isLocal={false} />
          ) : (
  ...
  return (
    <AkapuluProvider
      config={{
        ...
        // Optional connectBody / headers — see "Passing a body and headers" above.
      }}
    >
      <CustomCallUi />
    </AkapuluProvider>
...
| `useAkapuluSession` | Session state + `start` / `end` (`start()` also arms AudioContext for the connect chime) |
| `useAkapuluMediaControls` | Mic/cam state and toggles |
| `AkapuluBotAudio` | Plays bot audio track |
| `useConnectChime` | Plays a short chime when status goes `connecting` → `connected` |
| `armConnectAudio` / `playConnectChime` | Low-level Web Audio helpers used by the hook |
...
- [Prebuilt UI](/examples/web-sdk/prebuilt-ui) — [`Akapulu/prebuilt-ui`](https://github.com/Akapulu/prebuilt-ui): **`@akapulu/react`** (`AkapuluProvider` with local `connectPath` / `updatesPath`) plus **`@akapulu/react-ui`** (`AkapuluConversation` for the full default layout: video, transcript, controls). Useful when you want to ship quickly and customize via props or CSS rather than rebuilding layout.

- [Prebuilt UI (styled)](/examples/web-sdk/prebuilt-ui-styled) — [`Akapulu/prebuilt-ui-styled`](https://github.com/Akapulu/prebuilt-ui-styled): the same prebuilt path with a dark theme, a custom tool toast, and a post-call review screen (recording + transcript).

- [Customized UI](/examples/web-sdk/customized-ui) — [`Akapulu/customized-ui`](https://github.com/Akapulu/customized-ui): **`@akapulu/react` only**—same provider pattern, then **`useAkapuluSession`**, **`useAkapuluMediaControls`**, **`useAkapuluParticipantRoles`**, **`useConnectChime`**, **`useAkapuluEvents`**, **`AkapuluBotAudio`**, and **`@daily-co/daily-react`** (`DailyVideo`, track hooks) for your own chrome. Useful when you need full control over markup and state wiring.

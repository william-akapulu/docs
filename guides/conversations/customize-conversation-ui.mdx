...
| `durationWarning` | Banner on the video while the call is in its last 30 seconds | `akapulu-duration-warning` | `duration-warning` |
...
Fired once, about 30 seconds before a live call hits its duration cap. The prebuilt `AkapuluConversation` shows this as a banner on the video. The call stays connected. Rooms of 30 seconds or less do not send it.

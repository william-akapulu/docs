...
- **Bot participants**: Akapulu runtime participants that handle orchestration and assistant media in the room.
...
7. **Bot participants join the room**
   - After initialization completes, Akapulu runtime participants join and the assistant media becomes available in-room.
...
<Note>
The [Akapulu Web SDK](/web-sdk/overview) orchestrates:
...
If a user joins a call and reaches the session duration cap while the call is active, Akapulu automatically ends the session. In this case, the frontend should handle a timeout event and show a clear message to the user (for example, usage limit reached).
...
The [Custom UI](/examples/basic/custom-ui) example implements `conversation-details` and `view-recording` handlers that call these endpoints with the [Server SDK](/web-sdk/server-sdk) (`@akapulu/server`).

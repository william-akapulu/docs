...
`require_function_call` makes the assistant **call a function** on the next model turn in this node instead of answering with plain text. Use it when a stage must always invoke exactly one tool-for example a hand-off that must run a single transition or HTTP action before normal dialogue.

<Note>
**Constraint:** When `require_function_call` is `true`, the node must define **exactly one** function in `functions`.
</Note>
...
Open **Advanced Settings** on the node, then set **Force Function Call** to yes. While this is on, each function on the node shows a **Call required** indicator;

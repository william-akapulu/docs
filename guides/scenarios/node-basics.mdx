...
<Note>
**Loop protection:** If `require_function_call` is enabled and the assistant gets stuck repeatedly calling tools without replying in text, Akapulu automatically relaxes the forced-call requirement after 5 consecutive tool-only turns for that session/node. After that, the assistant can either call a tool or reply in plain text to prevent infinite loops.
</Note>

...
Use the toggle in the top-right corner of the scenario page to switch between visual mode and JSON mode. You can also send this JSON as `nodes_json` to [Create scenario](/api-reference/scenarios/create) and [Update scenario](/api-reference/scenarios/update).
...
All function types accept optional `filler_speech` (boolean). When `true`, connect adds a required `filler_speech` argument. The model's instruction for that argument is: "Extremely short spoken filler while this tool runs. A few words only, never more than one short sentence."
...
- `filler_speech` (optional boolean): when `true`, connect adds a required `filler_speech` argument.
...
- function keys are limited to `name`, `description`, `type`, `transition_to`, `allowed_next_nodes`, `endpoint_id`, `knowledge_base_id`, `parameters`, `require_reason`, and `filler_speech`
...
- `filler_speech`, if provided, must be a boolean
- `filler_speech` is valid on `transition`, `http`, `rag`, and `vision` functions
- HTTP templates cannot use an LLM variable named `filler_speech`

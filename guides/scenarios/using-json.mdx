...
- `require_function_call` (optional boolean): when `true`, the assistant must call the function specified for this node. Must be used with **exactly one** entry in `functions`. Defaults to omitted / off
...
- if `require_function_call` is `true`, the node must have **exactly one** function in `functions` (not zero, not more than one)

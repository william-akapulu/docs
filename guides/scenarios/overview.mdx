...
Saving an OpenAI key requires a paid plan. Models that do not need a key run on Akapulu Labs’ key.

## Testing the scenario

Use the **Testing Mode** tab in the Scenario Menu to run an **LLM-only, text-based simulation** of the scenario in the editor.

Each test session uses **0.25 credits** from the same pool as live conversation minutes (one live minute = **1 credit**) and [scripted clips](/guides/clips/overview).
...
- [Create scenario API](/api-reference/scenarios/create)
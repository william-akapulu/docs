...
Akapulu also stamps **`X-Akapulu-Conversation-Session-Id`** on every HTTP tool POST (live conversation and Testing Mode). That value is not a template. It is added at call time so your server can identify the session. If you set the same header in the endpoint config, Akapulu overwrites it. Use it to [update conversation context](/guides/conversations/updating-conversation-context) after a background job finishes.

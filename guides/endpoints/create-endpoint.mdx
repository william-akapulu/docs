...
This guide covers endpoint setup and attachment in both JSON and UI flows. You can also [create endpoints via the API](/api-reference/endpoints/create).
...
<Note>
Akapulu Labs invokes your endpoint with the **POST** method and a JSON-encoded body. **`Content-Type: application/json`** is sent by default; add **`Content-Type`** under **Headers** if you need a different value. Every tool POST also includes **`X-Akapulu-Conversation-Session-Id`** (live and Testing Mode). Use that header if your server needs to call back into the same conversation, including [Updating Conversation Context](/guides/conversations/updating-conversation-context). You cannot put the id in the endpoint template ahead of time.
</Note>

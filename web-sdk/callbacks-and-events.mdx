...
Fired once, about 30 seconds before a live call hits its duration cap. `AkapuluConversation` shows a red banner: the call is ending because the plan's session limit was reached, and to upgrade for longer calls and more minutes. The call stays connected. Rooms of 30 seconds or less do not send it.

<ParamField path="type" type='"duration_warning"'>
  Event discriminator.
</ParamField>

<ParamField path="secondsRemaining" type="number">
  Whole seconds until the call ends.
</ParamField>

...
### <span id="connect-conversation">`connectConversation(body)`</span>
...
<Note>
Your connect route starts a billed conversation. Keep `AKAPULU_API_KEY` on the server, and authenticate users on that route before starting a call. Demo apps often skip this; add it before production.
</Note>
...
**`AkapuluProvider`** (from [`@akapulu/react`](/web-sdk/react-sdk)) reads **`config.endpoints.connectPath`** and **`config.endpoints.updatesPath`**, sends a **POST** request to the connect URL, then **GET**s the updates URL only while the session is joining (until `call_is_ready`). Live transcripts after that come from Daily, not this poll. `config.headers` are sent on both requests. 
